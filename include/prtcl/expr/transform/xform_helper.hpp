...
protected:
  ...
  template <expr_kind K, typename... Ts>
  using expr_ref = expr<expr_kind::expr_ref, expr<K, Ts...>>;
  ...
  template <typename... Ts> using call_expr = expr<expr_kind::call, Ts...>;

  template <expr_kind K>
  static constexpr bool is_assign_v = K == expr_kind::assign;

  template <expr_kind K>
  static constexpr bool is_opassign_v =
      boost::hana::in(K, boost::hana::make_tuple(expr_kind::plus_assign,
                                                 expr_kind::minus_assign,
                                                 expr_kind::multiplies_assign,
  ...
  static constexpr bool is_bop_v = boost::hana::in(
      K, boost::hana::make_tuple(expr_kind::plus, expr_kind::minus,
