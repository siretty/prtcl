...
template <typename IndexT> constexpr auto unsign(IndexT);
...
private:
  template <typename T> using size_type_t = decltype(std::declval<T>().size());
...
public:
  ...
  template <typename IndexVecT>
  constexpr int compare(const IndexVecT &lhs, const IndexVecT &rhs) const {
    assert(lhs.size() == rhs.size());
    using size_type = size_type_t<IndexVecT>;
    ...
    for (size_type dim = 1; dim < lhs.size(); ++dim) {
      ...
      if (less_msb(unsign(lhs[msd]) ^ unsign(rhs[msd]),
...
private:
  ...
  template <typename T> constexpr bool less_msb(T lhs, T rhs) const {
  ...
  template <typename IndexT> constexpr auto unsign(IndexT index) const {
