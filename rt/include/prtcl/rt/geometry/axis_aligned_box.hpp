...
template <typename MathPolicy_, size_t N_> struct axis_aligned_box {
  ...
  using rvec = typename math_policy::template ndtype_t<dtype::real, N_>;
...
public:
  axis_aligned_box(
      rvec radius_ = c::template ones<dtype::real, N_>(),
      rvec center_ = c::template zeros<dtype::real, N_>())
      : _radius{radius_}, _center{center_} {}

public:
  template <typename Point> auto distance(Point &&point_) const {
    auto const zeros = c::template zeros<dtype::real, N_>();
...
private:
  rvec _radius = c::template ones<dtype::real, N_>();
  rvec _center = c::template zeros<dtype::real, N_>();
};
