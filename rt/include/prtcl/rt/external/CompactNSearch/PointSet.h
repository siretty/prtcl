...
public:
   ...
   */
  unsigned int
  neighbor(unsigned int point_set, unsigned int i, unsigned int k) const {
  ...
  auto &get_sort_table() const { return m_sort_table; }

private:
  friend NeighborhoodSearch;

private:
  static constexpr HashKey hash_key_init =
      HashKey::filled(std::numeric_limits<int>::lowest());

private:
  PointSet(
      Real const *x, std::size_t n, bool dynamic, void *user_data = nullptr)
      : m_x(x), m_n(n), m_dynamic(dynamic), m_user_data(user_data),
        m_neighbors(n), m_keys(n, hash_key_init) {
    m_old_keys = m_keys;
  }
  ...
  void resize(Real const *x, std::size_t n) {
    m_x = x;
    m_n = n;
    m_keys.resize(n, hash_key_init);
    m_old_keys.resize(n, hash_key_init);
...
template <typename T> void PointSet::sort_field(T *lst) const {
  ...
  std::transform(
      m_sort_table.begin(), m_sort_table.end(),
#ifdef _MSC_VER
