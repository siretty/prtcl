...
#include <prtcl/core/morton_order.hpp>
...
void NeighborhoodSearch::z_sort() {
  for (PointSet &d : m_point_sets) {
    ...
    std::sort(
        d.m_sort_table.begin(), d.m_sort_table.end(),
        [&](unsigned int a, unsigned int b) {
          auto ci_a = cell_index(d.point(a)), ci_b = cell_index(d.point(b));
          return prtcl::core::morton_order(ci_a.k, ci_b.k);
        });
  }
  m_initialized = false;
}
...
void NeighborhoodSearch::resize_point_set(
    unsigned int index, Real const *x, std::size_t size) {
...
void NeighborhoodSearch::find_neighbors(
    unsigned int point_set_id, unsigned int point_index, Fn_ fn) const {
  query(point_set_id, point_index, fn);
}

template <typename Fn_>
void NeighborhoodSearch::find_neighbors(Real const *x, Fn_ fn) const {
  query(x, fn);
}
...

  {
    ...
    while (it != m_map.end()) {
      ...
      if (kvp.second <= to_delete.front() && kvp.second >= to_delete.back() &&
          std::binary_search(
              to_delete.rbegin(), to_delete.rend(), kvp.second)) {
        it = m_map.erase(it);
      } else {
        ++it;
      }
       ...
       [&](std::pair<HashKey const, unsigned int> const *kvp_) {
         ...
         for (unsigned int a = 0; a < entry.n_indices(); ++a) {
           ...
           for (unsigned int b = a + 1; b < entry.n_indices(); ++b) {
             ...
             if (!m_activation_table.is_active(
                     va.point_set_id, vb.point_set_id) &&
                 !m_activation_table.is_active(
                     vb.point_set_id, va.point_set_id)) {
               continue;
             }
...
void NeighborhoodSearch::query(
    unsigned int point_set_id, unsigned int point_index, Fn_ fn) const {
  PointSet const &d = m_point_sets[point_set_id];
  ...
  std::pair<HashKey const, unsigned int> const &kvp = *it;
  HashEntry const &entry = m_entries[kvp.second];
  ...
  for (unsigned int b = 0; b < entry.n_indices(); ++b) {
    ...
    if ((point_set_id != vb.point_set_id) || (point_index != vb.point_id)) {
      ...
      PointSet const &db = m_point_sets[vb.point_set_id];
      Real const *xb = db.point(vb.point_id);
      Real tmp = xa[0] - xb[0];
      Real l2 = tmp * tmp;
      tmp = xa[1] - xb[1];
      ...
      if (l2 < m_r2) {
        fn(vb.point_set_id, vb.point_id);
...
template <typename Fn_>
void NeighborhoodSearch::query(Real const *xa, Fn_ fn) const {
  // neighbors.resize(m_point_sets.size());
  // for (unsigned int j = 0; j < m_point_sets.size(); j++) {
  //  auto &n = neighbors[j];
  //  n.clear();
