...
template <typename ModelPolicy_> class basic_fcc_lattice_source {
public:
...
public:
  basic_fcc_lattice_source() = delete;

  basic_fcc_lattice_source(basic_fcc_lattice_source const &) = default;
  basic_fcc_lattice_source &
  operator=(basic_fcc_lattice_source const &) = default;

  basic_fcc_lattice_source(basic_fcc_lattice_source &&) = default;
  basic_fcc_lattice_source &operator=(basic_fcc_lattice_source &&) = default;
  ...
  auto operator()(virtual_scheduler<real> &scheduler_, duration delay) {
    ...
    if constexpr (N == 3) {
      ...
      case 1:
        ...
        break;
      case 2:
        log::debug("app", "source", "fcc lattice layer c");
        loffset = rvec2{0, std::sqrt(3.0L) * g / 3};
        break;
      }
