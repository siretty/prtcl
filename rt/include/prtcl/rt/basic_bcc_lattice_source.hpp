...
template <typename ModelPolicy_> class basic_bcc_lattice_source {
public:
...
private:
  ...
  // between 0 and 1
  static constexpr auto height_factor = 0.5L;
  static constexpr auto compute_height(real h) {
    return (1 + height_factor * (std::sqrt(2.0L) - 1)) * h;
  }

public:
  basic_bcc_lattice_source() = delete;

  basic_bcc_lattice_source(basic_bcc_lattice_source const &) = default;
  basic_bcc_lattice_source &
  operator=(basic_bcc_lattice_source const &) = default;

  basic_bcc_lattice_source(basic_bcc_lattice_source &&) = default;
  basic_bcc_lattice_source &operator=(basic_bcc_lattice_source &&) = default;
  ...
  basic_bcc_lattice_source(
    ...
    real const height = compute_height(h) / 2;
...
public:
  auto operator()(virtual_scheduler<real> &scheduler_, duration delay) {
    ...
    real const grid_diagonal = std::sqrt(4 * h * h - height * height);
    real const grid_side = grid_diagonal / std::sqrt(2.0L);
    ...
    if constexpr (N == 3) {
      ...
      rvec2 const coffset{grid_side, 0}, roffset{0, grid_side};
      // offset for the current layer for planar (triangular) grid
      rvec2 loffset{0, 0};
      switch (_age % 2) {
      case 0:
        log::debug("app", "source", "fcc lattice layer a");
        break;
      case 1:
        log::debug("app", "source", "fcc lattice layer b");
        loffset = rvec2{grid_side / 2, grid_side / 2};
        break;
      }
