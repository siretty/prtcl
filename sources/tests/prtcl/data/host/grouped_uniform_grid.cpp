...
TEST_CASE("prtcl/data/host/grouped_uniform_grid",
          "[prtcl][data][host][grouped_uniform_grid]") {
  using T = float;
  ...
  prtcl::grouped_uniform_grid<T, N> grid;
  grid.set_radius(1.5f);

  grid.update(mg);

  grid.neighbours(
      2, 0, mg, [](auto gi, auto ri) { std::cout << gi << " " << ri << "\n"; });
}
