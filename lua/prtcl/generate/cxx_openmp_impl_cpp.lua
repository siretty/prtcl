...
local function scheme_registration(o, S, T, N, K)
  o:iput('(void)(' .. S .. '<' .. T .. ', ' .. N .. ', prtcl::math::' .. K .. '>::registered_);'):nl()
end
...
function module.generate(prtcl, printer, namespaces, scheme_name)
  ...
  o:put('#include "' .. scheme_name .. '.hpp"'):nl()
  o:nl()
  o:put('#include "../math/kernel/cubic_spline_kernel.hpp"'):nl()
  o:nl()
  ...
  for _, scheme in ipairs(prtcl.schemes) do
    ...
    o:iput('void Register_' .. scheme_name .. '() {'):nl()
    o:increase_indent()
    scheme_registration(o, scheme_name, 'float', '3', 'cubic_spline_kernel')
    scheme_registration(o, scheme_name, 'double', '3', 'cubic_spline_kernel')
    o:decrease_indent()
    o:iput('}'):nl()
    o:nl()
  end
  ...
  for _, scheme in ipairs(prtcl.schemes) do
    if scheme_name == nil or scheme_name == scheme.name then
      ...
      o:nl()
      o:iput('using kernel_type = K<T, N>;')
      o:nl()
      ...
      -- {{{ method load(model)
      cxx_access_modifier(o, 'public')
      o:iput('void load(Model &model) {'):nl()
      o:increase_indent()
