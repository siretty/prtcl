...
#pragma omp for
        for (size_t i = 0; i < p._count; ++i) {
          ...
          p.density[i] = static_cast<nd_dtype_t<nd_dtype::real>>(0);
          ...
          p.pressure[i] = (p.compressibility[0] * o::max(static_cast<nd_dtype_t<nd_dtype::real>>(0), ((p.density[i] / p.rest_density[0]) - static_cast<nd_dtype_t<nd_dtype::real>>(1))));
        }
      }
    } // pragma omp parallel
  }
...
#pragma omp for
        for (size_t i = 0; i < p._count; ++i) {
          ...
          for (auto &n : _data.by_group_type.fluid) {
            for (auto const j : neighbors[n._index]) {
              p.acceleration[i] += ((p.viscosity[0] * (n.mass[j] / n.density[j]) * o::dot((p.velocity[i] - n.velocity[j]), (p.position[i] - n.position[j])) / (o::norm_squared((p.position[i] - n.position[j])) + (static_cast<nd_dtype_t<nd_dtype::real>>(0.01) * g.smoothing_scale[0] * g.smoothing_scale[0]))) * o::kernel_gradient_h((p.position[i] - n.position[j]), g.smoothing_scale[0]));
          ...
          for (auto &n : _data.by_group_type.boundary) {
            for (auto const j : neighbors[n._index]) {
              p.acceleration[i] += ((p.viscosity[0] * n.volume[j] * o::dot((p.velocity[i] - n.velocity[j]), (p.position[i] - n.position[j])) / (o::norm_squared((p.position[i] - n.position[j])) + (static_cast<nd_dtype_t<nd_dtype::real>>(0.01) * g.smoothing_scale[0] * g.smoothing_scale[0]))) * o::kernel_gradient_h((p.position[i] - n.position[j]), g.smoothing_scale[0]));

              p.acceleration[i] -= (static_cast<nd_dtype_t<nd_dtype::real>>(0.7) * n.volume[j] * p.rest_density[0] * (static_cast<nd_dtype_t<nd_dtype::real>>(2) * p.pressure[i] / (p.density[i] * p.density[i])) * o::kernel_gradient_h((p.position[i] - n.position[j]), g.smoothing_scale[0]));
            }
          }
        }
      }
