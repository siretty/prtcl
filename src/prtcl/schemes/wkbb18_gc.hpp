...
class wkbb18_gc : public SchemeBase {
public:
  using real = T;
...
public:
  wkbb18_gc() {
    this->RegisterProcedure("compute_diagonal", &wkbb18_gc::compute_diagonal);
    this->RegisterProcedure(
        "accumulate_acceleration", &wkbb18_gc::accumulate_acceleration);
  }
              ...
              for (auto const j : neighbors[n.index]) {
                ...
                p.v_sp[i] +=
                    ((((static_cast<T>(10) * (*p.u_mu / p.v_rho[i])) *
                     ...
                     o::outer_product(
                         (p.v_L[i] * o::kernel_gradient_h<kernel_type>(
    ...
    foreach fluid particle f {
      ...
      foreach boundary neighbor f_b {
        compute sp.f +=
          ...
          *
            outer_product(
              L.f * kernel_gradient_h(x.f - x.f_b, h),
              x.f - x.f_b
            )
        ;
      }
...
private:
  static inline bool const registered_ =
      GetSchemeRegistry().RegisterScheme<wkbb18_gc>(GetFullNameImpl());

  friend void Register_wkbb18_gc();
};
} // namespace schemes
} // namespace prtcl
#if defined(__GNUG__)
