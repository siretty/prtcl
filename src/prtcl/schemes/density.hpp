...
class density : public SchemeBase {
public:
  using real = T;
...
public:
  density() {
    this->RegisterProcedure("compute_density", &density::compute_density);
  }
...
public:
  void compute_density(Neighborhood const &nhood) {
    auto &g = _data.global;
...
private:
  static inline bool const registered_ =
      GetSchemeRegistry().RegisterScheme<density>(GetFullNameImpl());

  friend void Register_density();
};
} // namespace schemes
} // namespace prtcl
#if defined(__GNUG__)
