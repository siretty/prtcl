...
public:
  sesph() {
    this->RegisterProcedure(
        "compute_density_and_pressure", &sesph::compute_density_and_pressure);
    this->RegisterProcedure(
        "accumulate_acceleration", &sesph::accumulate_acceleration);
  }
  ...
  void Load(Model &model) final {
    ...
    for (size_t group_index = 0; group_index < group_count; ++group_index) {
      ...
      if (groups_fluid_data::selects(group)) {
        ...
        data.u_kappa = group.AddUniformFieldImpl<real>("compressibility");
...
public:
  void compute_density_and_pressure(Neighborhood const &nhood) {
    auto &g = _data.global;
            ...
            { // foreach boundary neighbor f_b
              for (auto &n : _data.groups.boundary) {
                for (auto const j : neighbors[n.index]) {
                  // compute
                  p.v_a[i] -=
                      ((((static_cast<T>(0.7) * n.v_V[j]) * *p.u_rho0) *
...
private:
  static inline bool const registered_ =
      GetSchemeRegistry().RegisterScheme<sesph>(GetFullNameImpl());

  friend void Register_sesph();
};
} // namespace schemes
} // namespace prtcl
#if defined(__GNUG__)
